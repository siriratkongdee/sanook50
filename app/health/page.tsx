export default function Health() {
  const articles = [
    { title: "Should I Try HRT? What the Experts Actually Say", href: "/health/hrt", excerpt: "I watched 8 hours of podcasts from Peter Attia, Mary Claire Haver & Rhonda Patrick so you don't have to. Here's the honest breakdown.", emoji: "🌿", tag: "Hormones" },
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#FAF4EC' }}>
      {/* Page title - cream bg, colored accent */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {articles.map((article, i) => (
            <a key={i} href={article.href || '#'} className="block rounded-2xl p-6 shadow-md hover:shadow-xl transition-all duration-300 border" style={{ backgroundColor: 'white', borderColor: '#e8ddd0' }}>
              ...
              <button className="mt-4 font-medium transition-colors hover:opacity-70" style={{ color: '#7D9B76' }}>Read more →</button>
            </a>
          ))}
        </div>
      </div>
